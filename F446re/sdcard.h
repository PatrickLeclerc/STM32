...
#define SDIO_DCTRL_OFFSET             ((uint32_t)(&(SDIO1->DCTRL)) - PERIPH_BASE)
#define SDIO_DCTRL_DTEN_BN            0U // DTEN
#define SDIO_DCTRL_DTEN_BB            *(__IO uint32_t *)(PERIPH_BB_BASE + (SDIO_DCTRL_OFFSET << 5) + (SDIO_DCTRL_DTEN_BN << 2))

// Alias word address of bits of the SDIO CLKCR register
#define SDIO_CLKCR_OFFSET             ((uint32_t)(&(SDIO1->CLKCR)) - PERIPH_BASE)
#define SDIO_CLKCR_CLKEN_BN           8U // CLKEN
#define SDIO_CLKCR_CLKEN_BB           *(__IO uint32_t *)(PERIPH_BB_BASE + (SDIO_CLKCR_OFFSET << 5) + (SDIO_CLKCR_CLKEN_BN << 2))
#endif // STM32L1XX_HD


// SDIO power supply control bits
#define SD_PWR_OFF                    ((uint32_t)0x00000000U) // Power off: the clock to card is stopped
#define SD_PWR_ON                     (SDIO_POWER_PWRCTRL)   // Power on: the card is clocked
